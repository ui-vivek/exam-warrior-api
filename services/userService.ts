...
export async function updateUserProfile(userId: string, data: { name?: string, examType?: string, preferredLanguage?: string, appLanguage?: string, state?: string, avatar?: string }) {
  const user = await User.findByIdAndUpdate(
    userId,
    { $set: data },
    { new: true }
